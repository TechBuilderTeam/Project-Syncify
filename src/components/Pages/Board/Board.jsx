...
const Board = () => {
    ...
    return (
        <div className="">
            ...
            {!loading && boards?.length === 0 ? <h3 className="text-center text-xl font-semibold">No Board Found, Go to Plan and Create New Board after assigning Team Leader</h3>

                :
                <div className="overflow-x-auto shadow-xl rounded w-full">
                    <table className="table w-full">
                        <thead>
                            <tr className=" text-sm text-[#2c01a1] dark:text-[#73e9fe]">
                                <th> Plans Name </th>
                                <th>Board Name</th>
                        ...
                        <tbody>
                            {!loading && boards?.map((board, idx) => (
                                <>
                                    <tr key={idx} className=" text-sm text-black dark:text-white">
                                        <td>{board?.timeline_name}</td>
                                        <td>{board?.name}</td>
                                        <td className="text-center">
                                            <div className="flex items-center">
                                                <img src={board?.assign?.image} alt="" className="w-8 h-8 rounded-full" />
                                            </div>
                                        </td>
                                        <td className="font-semibold">
                                        {board?.assign?.first_name + " " + board?.assign?.last_name}
                                        </td>
                                        <td>
                                            <button className="mx-4" onClick={() => handleDeleteButton(board.id)}>
                                                <MdDeleteForever className="text-xl" title="Delete" />
                                            </button>
                                        </td>
                                        <td>
                                            <button className="font-bold px-4 py-2 rounded-md" onClick={() => handleOpenDialog(board, "createTask")}>
                                                <MdAddTask className="text-xl" title="Add Task" />
                                            </button>
                                        ...
                                        <td>
                                            <button onClick={() => handleToggle(board.id)} className="mx-4 px-2 py-1 rounded">
                                                {expandedBoardId === board.id ? <BiCollapseVertical className="text-xl" title="show Less" /> : <HiMiniChevronUpDown className="text-xl" title="click to show more" />}
                                            </button>
                                        </td>
                                    </tr>
                                    ...
                                    {expandedBoardId === board.id && (
                                        <tr className="text-center text-sm text-black dark:text-white w-full">
                                            <td colSpan="10" className="w-full px-10">
