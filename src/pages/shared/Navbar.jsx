...
const Navbar = ({ handleThemeChange }) => {
    ...
    return (
        <div className=" mx-auto sm:max-w-xl md:max-w-full lg:max-w-screen-xl p-4 "  >
            <div className="flex justify-between items-center gap-10  backdrop-filter backdrop-blur-3xl mb-10 fixed top-0 left-0 z-50 w-full h-20 px-6">
                ...
                <div className="hidden w-2/4  md:flex  justify-start items-center gap-4 font-semibold text-lg text-[#8401A1] dark:text-[#73e9fe]" >
                 ...
                 {/** toggle section */}
                <div className="w-1/3 hidden  md:flex justify-between items-center gap-2 text-[#8401A1] dark:text-[#73e9fe] font-semibold text-lg">
                ...
                {/** start toggle section and navlist for mobile device */}
                   ...
                   <div>
                        <label className="swap swap-rotate flex   lg:hidden mt-3">
                   ...
                   {/** toggle bar button */}
                    <div className="flex justify-center items-center lg:hidden" onClick={toggleMenu}>
                        ...
                        <div>
                            {open ? <FaXmark className="text-white text-4xl cursor-pointer"/> : <HiMenuAlt2 className="text-white text-4xl cursor-pointer"/> }
                        </div>
                        
                    </div>
                    
                    {/** conditional show navlist items */}
                    <div className={`${open ? "flex" : "hidden"} w-full h-fit bg-gradient-to-b from-[#9d11bd] to-[#73e9fe] p-4 absolute top-[80px] left-0`}>
         ...
         //Create new Navbar section
        // <div className='shadow-md w-full fixed top-0 left-0'>
        //    <div className='md:flex items-center justify-between bg-white py-4 md:px-10 px-7'>
        //     {/* logo section */}
        //     <div className='font-bold text-2xl cursor-pointer flex items-center gap-1'>
        //         <HiMenuAlt2 className='w-7 h-7 text-blue-600'/>
        //         <span>Inscribe</span>
        //     </div>
        //     {/* Menu icon */}
        //     <div onClick={()=>setOpen(!open)} className='absolute right-8 top-6 cursor-pointer md:hidden w-7 h-7'>
        //         {
        //             open ? <FaXmark /> : <HiMenuAlt2 />
        //         }
        //     </div>
        //     {/* linke items */}
        ...
        // <div className="w-full flex bg-black justify-betweeen items-center gap-1 lg:px-16 px-6 py-3 sticky top-0 z-50 ">
        ...
        //       {
        //                 Links.map(({name,path}) => {
        //                     <Link key={path} to={path}className="text-white uppercase font-semibold cursor-pointer p-3 rounded-lg hover:bg-orange-600 hover:text-white w-full text-center" spy={true} offset={-100} smooth={true}>{name}</Link>
        //                 })
        //             }
        //     </ul>

        //     <button className="bg-orange-600 text-white px-10 py-13 rounded-full font-bold transform hover:scale-105 transition-transform duration-300">Order Now</button>

            // {/** mobile menu starts */}
            // <div className="flex justify-center items-center lg:hidden mt-3 " onClick={toggleMenu}>
            //     <div>
            //         {open ? <FaXmark className="text-white text-3xl cursor-pointer"/> : <HiMenuAlt2 className="text-white text-3xl cursor-pointer"/> }
            //     </div>
            // </div>

            // <div className={`${open ? "flex" : "hidden"} w-full h-fit bg-black p-4 absolute top-[80px] left-0`}>
